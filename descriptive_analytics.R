...
# Theft And Handling
p11 <- ggplot(nationaldf,aes(x=date,y=N_THC))+geom_bar(stat='identity', fill='darkblue') + ggtitle('National number of theft and handling convictions from July 2015 to March 2018')+ 
...
subset(timeseries_df, court!='Metropolitan & City') %>%
  ggplot( aes(x=date, y=P_MOC, group=court, fill=court)) +
  geom_area(stat='identity') +
  ggtitle('') +
  scale_x_date(limits = as.Date(c('2015-07-01', '2018-3-01')), 
